...
- The menu bar has eight menus, in this order: **File, Edit, Astrometry, Images, Tools,
  Internet, Windows, Help**. In Portuguese: **Arquivo, Editar, Astrometria, Imagens,
  Ferramentas, Internet, Janelas, Ajuda**.
...
- Some items are greyed out (disabled) until you do an earlier step. For example, most
  Astrometry items need loaded images first.
...
- A barra de menus tem oito menus, nesta ordem: **Arquivo, Editar, Astrometria, Imagens,
  Ferramentas, Internet, Janelas, Ajuda** (em inglês: File, Edit, Astrometry, Images, Tools,
  Internet, Windows, Help).
...
- Alguns itens ficam cinza (desabilitados) até você fazer um passo anterior. Por exemplo, a
  maior parte do menu Astrometria precisa de imagens carregadas antes.
...
| Save as FITS... | Salvar como FITS... | — | Same, for the active image only. | O mesmo, só para a imagem ativa. | 🟡 |
| Export Image to... ▸ | Exportar Imagem como... ▸ | — | Submenu: **JPEG / PNG / BMP…**, **JPEG (quick)…** (PT **JPEG (rápido)…**), **PNG…**. Saves the active image as you see it on screen (a picture, not science data). | Submenu: **JPEG / PNG / BMP…**, **JPEG (rápido)…**, **PNG…**. Salva a imagem ativa como aparece na tela (uma figura, não dado científico). | 🟢 |
...
| Reset Files | Resetar Arquivos | `Ctrl+R` | Closes all images and clears the session. **Does not ask to save.** | Fecha todas as imagens e limpa a sessão. **Não pergunta se quer salvar.** | 🟢 |
| Fechar Imagens (PT in both languages) | Fechar Imagens | `Ctrl+W` | Exactly the same as Reset Files. **Does not ask to save.** | Exatamente o mesmo que Resetar Arquivos. **Não pergunta se quer salvar.** | 🟢 |
| Reload MPCOrb | Recarregar MPCOrb | — | Re-reads the offline asteroid list `MPCORB.DAT`. If it is missing, the log says to use **Internet → Download MPCOrb**. | Relê a lista offline de asteroides `MPCORB.DAT`. Se não existir, o registro manda usar **Internet → Baixar MPCOrb**. | 🟡 |
...
🇬🇧 **English** — How the image looks on screen and information about it. None of these
change your original files.

🇧🇷 **Português** — Como a imagem aparece na tela e informações sobre ela. Nada disso altera
seus arquivos originais.

| EN label | PT label | Shortcut | What it does | O que faz | Level |
|---|---|---|---|---|---|
| Edit Image Parameters... | Editar Parâmetros da Imagem... | — | Edit the active image's object name, filter, observer, telescope, RA/Dec, Julian Date, exposure, plate scale ("/px), gain and saturation. | Edita da imagem ativa: nome do objeto, filtro, observador, telescópio, AR/Dec, Data Juliana, exposição, escala ("/px), ganho e saturação. | 🟡 |
| Display Header... | Exibir Cabeçalho... | — | Shows the FITS header (the text block of information stored in the file). Does nothing for files that are not FITS. | Mostra o cabeçalho FITS (o bloco de texto com informações guardado no arquivo). Não faz nada em arquivos que não são FITS. | 🟡 |
...
| Re-Stack Images | Re-empilhar Imagens | — | Aligns all images on the stars and combines them (Average / Median / Add). Opens `stacked.fits`. | Alinha todas as imagens pelas estrelas e combina (Média / Mediana / Soma). Abre `stacked.fits`. | 🟡 |
| Zoom In | Ampliar Mais | `Ctrl++`, `+`, `=` | Zooms in. | Aproxima. | 🟢 |
| Zoom Out | Ampliar Menos | `Ctrl+-`, `-` | Zooms out. | Afasta. | 🟢 |
| Fit Window Size | Ajustar ao Tamanho da Janela | `Ctrl+F`, `0` | Fits the whole image in its window. | Encaixa a imagem inteira na janela. | 🟢 |
...
| Invert Display | Inverter Exibição | `Ctrl+I` | On/off. Shows black stars on a white sky. | Liga/desliga. Mostra estrelas pretas em céu branco. | 🟢 |
...
| Magnifying Glass | Lupa | `Ctrl+G` | On/off. A zoomed loupe follows the cursor on all images. | Liga/desliga. Uma lupa acompanha o cursor em todas as imagens. | 🟢 |
...
🇬🇧 **English** — Blinking, overlays and analysis windows.

🇧🇷 **Português** — Piscar, sobreposições e janelas de análise.

| EN label | PT label | Shortcut | What it does | O que faz | Level |
|---|---|---|---|---|---|
| Blink Images | Piscar Imagens | `Ctrl+B` | Opens the Blink View: shows the images one after the other so a moving object "jumps". Needs 2 or more images. See [Blink view](#blink-view--visão-de-piscar). | Abre a Visão de piscar: mostra as imagens uma após a outra para um objeto em movimento "pular". Precisa de 2 ou mais imagens. | 🟢 |
...
| EN label | PT label | Shortcut | What it does | O que faz | Level |
|---|---|---|---|---|---|
| Download MPCOrb | Baixar MPCOrb | — | Downloads `MPCORB.DAT` (the Minor Planet Center list of all known asteroid orbits) so known objects can be found offline. The confirmation says about 200 MB; the status-bar tip says ~500 MB (the text is inconsistent). | Baixa o `MPCORB.DAT` (lista do Minor Planet Center com as órbitas de todos os asteroides conhecidos) para achar objetos conhecidos offline. A confirmação diz cerca de 200 MB; a dica na barra de status diz ~500 MB (o texto é inconsistente). | 🟡 |
| Update MPCOrb | Atualizar MPCOrb | — | Downloads the daily update and adds the new objects. Needs `MPCORB.DAT` already downloaded. | Baixa a atualização diária e acrescenta os objetos novos. Precisa do `MPCORB.DAT` já baixado. | 🟡 |
...
🇬🇧 **English** — Arranging windows, the colour theme, and showing/hiding toolbars and panels.

🇧🇷 **Português** — Organizar janelas, o tema de cores e mostrar/esconder barras e painéis.

| EN label | PT label | Shortcut | What it does | O que faz | Level |
|---|---|---|---|---|---|
| Tile Windows | Organizar em Grade | — | Arranges the image windows side by side. | Organiza as janelas de imagem lado a lado. | 🟢 |
| Cascade Windows | Cascata de Janelas | — | Stacks them diagonally. | Empilha na diagonal. | 🟢 |
...
> 💡 **Tip / Dica**
> 🇬🇧 Closed a panel by accident? Bring it back from this **Windows** menu.
> 🇧🇷 Fechou um painel sem querer? Traga de volta por este menu **Janelas**.
...
| FITS Header… | Cabeçalho FITS… | Same as **Images → Display Header...** | Igual a **Imagens → Exibir Cabeçalho...** | 🟡 |
...
(TIFF, RAW, PDS, …) are ignored when dropped: use **File → Load Images...** for those.
...
TAR/7Z/RAR; outros formatos (TIFF, RAW, PDS, …) são ignorados: use **Arquivo → Carregar
Imagens...** para eles.
...
(quoted below). Show or hide them in the **Windows** menu.

🇧🇷 **Português** — Três barras só com ícones. Pare o mouse sobre um ícone para ver a dica
(citada abaixo). Mostre ou esconda pelo menu **Janelas**.
...
| 3 | Load Images (Ctrl+L) | Carregar Imagens (Ctrl+L) | File → Load Images... |
...
| 6 | Data Reduction (Ctrl+A) | Redução de Dados (Ctrl+A) | Astrometry → Data Reduction... |
| 7 | Moving Object Detection (Ctrl+M) | Detecção de Objetos em Movimento (Ctrl+M) | Astrometry → Moving Object Detection... |
| 8 | Stack Images (Ctrl+T) | Empilhar Imagens (Ctrl+T) | Astrometry → Stack Images... |
...
| 11 | Close all image windows | Fechar todas as janelas de imagem | Clears the session (like Reset Files). |
| 12 | Close all windows | Fechar todas as janelas | Windows → Close all Windows |

### Display / Exibição

| # | Tooltip EN | Tooltip PT | Does |
|---|---|---|---|
| 1 | Adjust background and display range | Ajustar fundo e intervalo de exibição | Images → Background and Range... |
| 2 | Select markings | Selecionar marcações | Images → Select Markings... |
| 3 | Invert display (Ctrl+I) | Inverter exibição (Ctrl+I) | Images → Invert Display |
| 4 | Known Object Overlay (Ctrl+K) | Sobreposição de objetos conhecidos (Ctrl+K) | Tools → Known Object Overlay |
| 5 | Zoom In | Ampliar | Zoom in |
| 6 | Zoom Out | Reduzir | Zoom out |
| 7 | Fit image to window (Ctrl+F) | Ajustar imagem à janela (Ctrl+F) | Images → Fit Window Size |

### Blink / Piscar

| # | Tooltip EN | Tooltip PT | Does |
|---|---|---|---|
| 1 | Blink Images (Ctrl+B) | Piscar Imagens (Ctrl+B) | Tools → Blink Images |
...
| 3 | Stop Blinking (Ctrl+F9) | Parar Piscar (Ctrl+F9) | Tools → Stop Blinking |
...
|---|---|---|---|---|---|
| Workflow | Fluxo de Trabalho | Left, visible at start | Six numbered steps (Load Images, Data Reduction, Known Object Overlay, Blink Images, Measure Objects, ADES Report). ○ = not yet, ▶ = next step, ✓ = done. The next step has a **▶ Do it** button. The **Fluxo automático** box (PT in both languages) runs steps 2, 3 and 4 by itself after loading. | Seis passos numerados (Carregar Imagens, Redução de Dados, Obj. Conhecidos, Piscar Imagens, Medir Objetos, Relatório ADES). ○ = ainda não, ▶ = próximo passo, ✓ = feito. O próximo passo tem o botão **▶ Executar**. A caixa **Fluxo automático** executa sozinha as etapas 2, 3 e 4 depois de carregar. | 🟢 |
...
- **Left: the step message.** It tells you what to do next, for example "Ready — load images
  to start", "Step 1: Images loaded — Run Data Reduction", "Step 2 done — run Known Object
  Overlay (Ctrl+K)", "Step 3 done — start Blinking (Ctrl+B)", "Blinking — Space = pause,
...
- **À esquerda: a mensagem de etapa.** Diz o que fazer em seguida, por exemplo "Pronto",
  "Step 1: Images loaded — Run Data Reduction", "Step 2 done — run Known Object Overlay
...
🇬🇧 **English** — Opened with **Tools → Blink Images** (`Ctrl+B`). It fills the window and
...
🇧🇷 **Português** — Aberta com **Ferramentas → Piscar Imagens** (`Ctrl+B`). Ocupa a janela e
...
empty window says "No image loaded / Use File → Load Images…".
...
| `F1` | Help Contents | Conteúdo da Ajuda | Help menu |
...
| `Ctrl+.` | Stop Data Reduction | Parar Redução de Dados | Astrometry menu |
| `Ctrl++` | Zoom In | Ampliar Mais | Images menu |
| `Ctrl+-` | Zoom Out | Ampliar Menos | Images menu |
| `Ctrl+A` | Data Reduction | Redução de Dados | Astrometry menu |
| `Ctrl+B` | Blink Images | Piscar Imagens | Tools menu |
| `Ctrl+C` | Copy Image | Copiar Imagem | Edit menu |
| `Ctrl+E` | Ecliptic / Galactic Overlay | Sobreposição Eclíptica / Galáctica | Tools menu |
| `Ctrl+F` | Fit Window Size | Ajustar ao Tamanho da Janela | Images menu |
| `Ctrl+F9` | Stop Blinking | Parar Piscar | Tools menu |
...
| `Ctrl+R` | Reset Files (no save prompt) | Resetar Arquivos (sem perguntar) | File menu |
| `Ctrl+S` | Save Project | Salvar Projeto | File menu |
| `Ctrl+T` | Stack Images (Track & Stack) | Empilhar Imagens | Astrometry menu |
| `Ctrl+W` | Fechar Imagens (= Reset Files, no save prompt) | Fechar Imagens (sem perguntar) | File menu |
