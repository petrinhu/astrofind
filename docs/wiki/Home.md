...
| Load the image series | **File → Load Images...** (`Ctrl+L`) / **Arquivo → Carregar Imagens...** | Also opens SER, XISF, TIFF/PNG, DSLR RAW, NASA PDS and archives (ZIP, TAR, 7Z, RAR). `Ctrl+L` is also given to **Tools → Light Curve…**, so the key may do nothing: use the menu or the toolbar. |
...
| Carregar a série de imagens | **Arquivo → Carregar Imagens...** (`Ctrl+L`) / **File → Load Images...** | Também abre SER, XISF, TIFF/PNG, RAW de DSLR, NASA PDS e arquivos compactados (ZIP, TAR, 7Z, RAR). O `Ctrl+L` também está em **Ferramentas → Curva de Luz…**, então a tecla pode não fazer nada: use o menu ou a barra de ferramentas. |
