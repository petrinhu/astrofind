...
| "No image files found in ZIP: …" | The ZIP has no files of a type AstroFind extracts. BMP, JPEG and PDS4 `.xml` are not extracted from archives. | Extract the ZIP yourself and load the files with **File → Load Images…**. |
...
| "Nenhum arquivo de imagem encontrado no ZIP: …" | O ZIP não tem arquivos de um tipo que o AstroFind extrai. BMP, JPEG e `.xml` PDS4 não são extraídos de arquivos compactados. | Extraia o ZIP você mesmo e carregue os arquivos com **Arquivo → Carregar Imagens…**. |
...
| The window opens in a strange place, off screen, or with panels missing | The saved window layout does not fit your current screen. | Use **Windows → Arrange all Windows** and the **Windows → View …** items to show the panels again. If that does not help, reset the settings (see [below](#how-to-reset-the-settings--como-restaurar-as-configurações)). |
...
| "FITS file has no 2D image: …" | The FITS file has no picture, only tables or a header. | Use a FITS file with an image. Star tables can be loaded with **Astrometry → Import Detected Stars (DAOPHOT/SExtractor)…**. |
...
| You drop a TIFF, PNG, RAW or PDS file onto the window and nothing happens | Drag and drop accepts only `.fits`, `.fit`, `.fts`, ZIP and the other archive types. | Use **File → Load Images…** (`Ctrl+L`). |
| **File → Open Recent…** does not load all files of the folder | This list only looks for FITS, SER, XISF, TIFF, PNG, PDS3 and the common RAW types. It skips archives, BMP, JPEG and PDS4 `.xml`. | Use **File → Load Images…**. |
...
| **Images → Display Header…** does nothing | This item reads FITS headers only. | For other formats the information is in the Log panel line "Loaded: …" and in **Images → Edit Image Parameters…**. |
...
> ⚠️ **Watch out:** **File → Save as FITS…** copies the original file and adds the WCS. For a
...
| "Arquivo FITS não contém imagem 2D: …" | O arquivo FITS não tem imagem, só tabelas ou cabeçalho. | Use um FITS com imagem. Tabelas de estrelas podem ser carregadas em **Astrometria → Importar Estrelas Detectadas (DAOPHOT/SExtractor)…**. |
...
| Você solta um TIFF, PNG, RAW ou PDS na janela e nada acontece | Arrastar e soltar só aceita `.fits`, `.fit`, `.fts`, ZIP e os outros tipos compactados. | Use **Arquivo → Carregar Imagens…** (`Ctrl+L`). |
| **Arquivo → Abrir Recente…** não carrega todos os arquivos da pasta | Essa lista só procura FITS, SER, XISF, TIFF, PNG, PDS3 e os tipos RAW mais comuns. Ela ignora arquivos compactados, BMP, JPEG e `.xml` PDS4. | Use **Arquivo → Carregar Imagens…**. |
...
| **Imagens → Exibir Cabeçalho…** não faz nada | Esse item só lê cabeçalhos FITS. | Para outros formatos, a informação está na linha "Carregado: …" do painel de Log e em **Imagens → Editar Parâmetros da Imagem…**. |
...
> ⚠️ **Atenção:** **Arquivo → Salvar como FITS…** copia o arquivo original e acrescenta o WCS.
...
**Astrometry → Data Reduction…** (`Ctrl+A`). Each failed image gets a Log line
...
**Astrometria → Redução de Dados…** (`Ctrl+A`). Cada imagem que falha ganha uma linha no Log
...
| "ASTAP: nenhuma solução de placa encontrada" | O ASTAP não conseguiu casar as estrelas. Motivos comuns: nenhum banco de estrelas do ASTAP instalado, um banco que não serve para o tamanho do seu campo, ou uma posição errada no cabeçalho. | Instale um banco de estrelas do ASTAP adequado ao seu campo de visão (veja [hnsky.org](https://www.hnsky.org/astap.htm)). Confira AR/Dec em **Imagens → Editar Parâmetros da Imagem…**. |
...
🇬🇧 **English**
These messages come from **Tools → Known Object Overlay** (`Ctrl+K`), which downloads reference
...
| "No catalog stars found in field" | No catalog star in the field fits the magnitude limits, or the field centre is wrong. | Check "Catalog mag (faint):" (default 16) and "Catalog mag (bright):" (default 10) in **Settings → Detection**. Make sure Data Reduction solved image 1: without a solution the search uses the header position and a 0.5° radius. |
| You changed "VizieR mirror:" and nothing changed | The field shows "vizier.cfa.harvard.edu", but a value without `https://` is rejected, so AstroFind keeps using `https://tapvizier.cds.unistra.fr/TAPVizieR/tap/sync`. | Nothing to do; the default server is used. If you type another server, include `https://`. |
...
| "SkyBoT query failed: …" | The IMCCE SkyBoT service could not be reached. | Download MPCORB.DAT once with **Internet → Download MPCOrb**; AstroFind then uses it when SkyBoT is unreachable. |
| "MPCORB.DAT not found. Use Internet → Download MPCOrb." | The offline asteroid file is missing. | Use **Internet → Download MPCOrb** (about 200 MB). The file path is shown in **Settings → Connections** → "MPCORB.DAT:". **Internet → Update MPCOrb** needs the file to exist first. |
| No known objects at all, and no SkyBoT line in the log | Known objects are only searched when the image has a valid date (Julian Date above 2400000). | Check the date in **Images → Edit Image Parameters…** ("Julian Date:"). |
...
🇧🇷 **Português**
Estas mensagens vêm de **Ferramentas → Sobreposição de Objetos Conhecidos** (`Ctrl+K`), que baixa
...
| Você mudou "Espelho VizieR:" e nada mudou | O campo mostra "vizier.cfa.harvard.edu", mas um valor sem `https://` é recusado, então o AstroFind continua usando `https://tapvizier.cds.unistra.fr/TAPVizieR/tap/sync`. | Não precisa fazer nada; o servidor padrão é usado. Se digitar outro servidor, inclua `https://`. |
...
| "MPCORB.DAT não encontrado. Use Internet → Baixar MPCOrb." | Falta o arquivo offline de asteroides. | Use **Internet → Baixar MPCOrb** (cerca de 200 MB). O caminho do arquivo aparece em **Configurações → Conexões** → "MPCORB.DAT:". **Internet → Atualizar MPCOrb** precisa que o arquivo já exista. |
| Nenhum objeto conhecido, e nenhuma linha do SkyBoT no log | Objetos conhecidos só são buscados quando a imagem tem data válida (Data Juliana acima de 2400000). | Confira a data em **Imagens → Editar Parâmetros da Imagem…** ("Data Juliana:"). |
...
🇬🇧 **English**
During Data Reduction the Log panel shows "Image …: … stars found" for each image. If the number
is 0 or very small:
...
| A wrong pixel scale in **Settings → Camera**. The label says "°/px", but AstroFind uses the value as **arcseconds per pixel**. A value typed in degrees makes the "Minimum FWHM:" filter (which is also converted from arcseconds, although it says "px") throw away every star. | Leave "Pixel Scale X:" and "Pixel Scale Y:" at 0 ("Auto") unless you know the scale in arcsec/px. |
...
| Tools → Growth Curve… says "No stars detected in this image.\nRun plate solving or star detection first." | Run **Astrometry → Data Reduction…** first. |
...
| Uma escala de pixel errada em **Configurações → Câmera**. O rótulo diz "°/px", mas o AstroFind usa o valor como **segundos de arco por pixel**. Um valor digitado em graus faz o filtro "FWHM mínimo:" (que também é convertido de segundos de arco, embora diga "px") jogar fora todas as estrelas. | Deixe "Escala de pixel X:" e "Escala de pixel Y:" em 0 ("Automático"), a menos que você saiba a escala em arcsec/px. |
...
| Ferramentas → Curva de Crescimento… diz "Nenhuma estrela detectada nesta imagem.\nExecute o plate solving ou a detecção de estrelas primeiro." | Rode **Astrometria → Redução de Dados…** antes. |
...
| "Run Data Reduction first to detect stars in the images." (Moving Object Detection) | No stars were detected yet. | Run **Astrometry → Data Reduction…** (`Ctrl+A`) first. |
...
| Log shows "Photometry: no catalog stars matched — using instrumental mag" | No catalog star near the object could calibrate the brightness. | Run the Known Object Overlay (`Ctrl+K`) before measuring. The position is still valid. |
...
| "Execute a Redução de Dados primeiro para detectar estrelas nas imagens." (Detecção de Objetos em Movimento) | Ainda não há estrelas detectadas. | Rode **Astrometria → Redução de Dados…** (`Ctrl+A`) antes. |
...
| Problem / message | Cause | Fix |
|---|---|---|
| **File → View ADES Report File** is greyed out | It is enabled only after Data Reduction. | Run **Astrometry → Data Reduction…** first. |
...
| `obsTime` in the ADES report is about 1 minute (68 s) later than the real UTC mid-exposure, or grows each time you rerun Data Reduction | Bug AUD-CORR-15 in 1.1.0: Data Reduction adds ΔT (default 68 s) and "Time Offset:" to every image time on each run, and the report labels the result UTC | Set **Settings → Camera → ΔT (TT − UTC):** and **Settings → Observer → Time Offset:** to 0, reload the images, run Data Reduction once. See [Settings](https://github.com/petrinhu/astrofind/wiki/Settings) |
| The seconds in `obsTime` have an unexpected number of decimals | "Time Precision:" in **Settings → Observer** is labelled in hours, but its whole-number part is used as the number of decimals of the seconds (1.0 = tenths of a second). | Leave it at 1.0 unless you need another precision. |

🇧🇷 **Português**

| Problema / mensagem | Causa | Solução |
|---|---|---|
| **Arquivo → Ver Arquivo de Relatório ADES** está desativado | Ele só fica ativo depois da Redução de Dados. | Rode **Astrometria → Redução de Dados…** antes. |
...
| O `obsTime` do relatório ADES sai cerca de 1 minuto (68 s) depois do meio da exposição em UTC, ou aumenta cada vez que você roda a Redução de Dados de novo | Bug AUD-CORR-15 na 1.1.0: a Redução de Dados soma o ΔT (padrão 68 s) e o "Deslocamento de tempo:" ao horário de cada imagem a cada execução, e o relatório marca o resultado como UTC | Coloque **Configurações → Câmera → ΔT (TT − UTC):** e **Configurações → Observador → Deslocamento de tempo:** em 0, recarregue as imagens e rode a Redução de Dados uma vez. Veja [Configurações](https://github.com/petrinhu/astrofind/wiki/Settings) |
| Os segundos em `obsTime` têm um número inesperado de casas decimais | "Precisão de tempo:" em **Configurações → Observador** aparece em horas, mas a parte inteira dela é usada como número de casas decimais dos segundos (1,0 = décimos de segundo). | Deixe em 1,0, a menos que precise de outra precisão. |
...
| Problem | Cause | Fix |
|---|---|---|
| Almost black image | Normal: the screen cannot show the full brightness range. | **Images → Background and Range…** → **"⟳ Auto"**; try the "Asinh" or "Raiz Quadrada (Sqrt)" transfer function. |
| Almost white image | **Invert Display** is on, or the range is set too low. | Turn off **Images → Invert Display** (`Ctrl+I`); click "⟳ Auto". |
...
| Problema | Causa | Solução |
|---|---|---|
| Imagem quase preta | Normal: a tela não consegue mostrar toda a faixa de brilho. | **Imagens → Fundo e Intervalo…** → **"⟳ Auto"**; experimente a função de transferência "Asinh" ou "Raiz Quadrada (Sqrt)". |
| Imagem quase branca | **Inverter Exibição** está ligado, ou o intervalo está baixo demais. | Desligue **Imagens → Inverter Exibição** (`Ctrl+I`); clique em "⟳ Auto". |
...
| Problem | Cause | Fix |
|---|---|---|
| `Ctrl+L` does nothing | `Ctrl+L` is assigned to both **File → Load Images…** and **Tools → Light Curve…**. When one key has two actions, Qt may run neither. | Use the menu or the "Load Images (Ctrl+L)" toolbar button. |
| `Ctrl+Shift+T` does not change the theme | It is assigned twice (menu item and toolbar button). | Click the moon/sun icon on the toolbar, or use **Windows → Toggle Day/Night Mode**, or **Settings → Display → Theme:**. |
...
| `Ctrl+A` does not select all text | In AstroFind `Ctrl+A` starts **Data Reduction**. | Use the mouse to select text. |
...
| Images disappeared after `Ctrl+W` or `Ctrl+R` | `Ctrl+W` (**File → Fechar Imagens**) and `Ctrl+R` (**File → Reset Files**) close all images **without asking** to save. | Reopen the images or your saved `.gus` project with **File → Open Project…**. Save often with `Ctrl+S`. |
...
| Problema | Causa | Solução |
|---|---|---|
| `Ctrl+L` não faz nada | `Ctrl+L` está ligado a **Arquivo → Carregar Imagens…** e a **Ferramentas → Curva de Luz…**. Quando uma tecla tem duas ações, o Qt pode não rodar nenhuma. | Use o menu ou o botão "Carregar Imagens (Ctrl+L)" da barra de ferramentas. |
| `Ctrl+Shift+T` não muda o tema | O atalho está em dois lugares (item de menu e botão da barra). | Clique no ícone de lua/sol da barra, ou use **Janelas → Alternar modo Dia/Noite**, ou **Configurações → Exibição → Tema:**. |
...
| As imagens sumiram depois de `Ctrl+W` ou `Ctrl+R` | `Ctrl+W` (**Arquivo → Fechar Imagens**) e `Ctrl+R` (**Arquivo → Resetar Arquivos**) fecham todas as imagens **sem perguntar** se quer salvar. | Abra as imagens de novo ou o seu projeto `.gus` com **Arquivo → Abrir Projeto…**. Salve sempre com `Ctrl+S`. |
...
- If the panel is hidden, use **Windows → View Log Panel**. **File → View Log File** does the
...
- Se o painel estiver escondido, use **Janelas → Exibir painel de log**. **Arquivo → Ver Arquivo
...
again, or Data Reduction will warn "Localização não configurada").
